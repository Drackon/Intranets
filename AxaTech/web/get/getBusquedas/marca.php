<?php
$idParam=json_decode($_POST['MarcParam']); 
// Check connection
...
if (!($conn->connect_error)) {
  //$query = "SELECT * FROM productos WHERE Marca = ?";   // $query is a simple variable (string)
   $query = "CALL BuscMarca(?)"; 
   $stmt = $conn->prepare($query);  // create a query statement 'controller' (variable name $stmt)
	...
	//*******************************************************************************
    $stmt->bind_param('s', $idParam->marca);  // 'i': integer
    $result=$stmt->execute();
